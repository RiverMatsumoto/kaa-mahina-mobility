...
struct SmTrialPlanner : public smacc2::SmaccStateMachineBase<SmTrialPlanner, MeasureState>
{
  ...
  virtual void onInitialize() override
  {
    ...
    this->getNode()->declare_parameter<double>("initial_bearing",
                                               0.0); // 0 is north, 45 degrees is northeast
    ...
    blackboard.gpsGrid =
        generate_gps_grid(initialLat, initialLon, initialBearing, gridLatLength, gridLonLength);
    this->setGlobalSMData("blackboard", blackboard);
    this->createOrthogonal<OrSubscribeMeasurement>();
    this->createOrthogonal<OrGetNextWaypoint>();
    this->createOrthogonal<OrLoweringMechanism>();
  }
};
...
#include <cr_mission_planner/states/st_get_next_waypoint.hpp>
